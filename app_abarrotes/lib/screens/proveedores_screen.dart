...
class _ProveedoresScreenState extends State<ProveedoresScreen> {
  ...
  Future<void> _openForm({Map<String, dynamic>? item, int? index}) async {
    final result = await showAppModal<Map<String, dynamic>>(
      context, title: item == null ? 'Nuevo proveedor' : 'Editar proveedor',
    ...
    try {
      ...
      await _load();
      if (mounted) showAppSnackbar(context, item == null ? 'Proveedor creado' : 'Proveedor actualizado', type: AppSnackbarType.success);
    } catch (e) { if (mounted) showAppSnackbar(context, 'Error: $e', type: AppSnackbarType.error); }
  }

  Future<void> _delete(int index) async {
    final item = _items[index];
    final confirmado = await showAppConfirmDialog(context, title: 'Eliminar proveedor', message: '¿Eliminar "${item['nombre']}"?');
    if (!confirmado) return;
    try {
      await _crud.delete(item['id']);
      await _load();
      if (mounted) showAppSnackbar(context, 'Proveedor eliminado', type: AppSnackbarType.error);
    } catch (e) { if (mounted) showAppSnackbar(context, 'Error: $e', type: AppSnackbarType.error); }
  }
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          : _items.isEmpty ? const Center(child: Text('No hay proveedores'))
          : ListView.builder(
...
class _ProveedorFormSheetState extends State<_ProveedorFormSheet> {
  ...
  void initState() {
    ...
    _contacto = TextEditingController(text: widget.initial?['contacto_nombre'] ?? '');
    _activo = widget.initial?['activo'] as bool? ?? true;
  }

  @override
  void dispose() { _codigo.dispose(); _nombre.dispose(); _ruc.dispose(); _direccion.dispose(); _telefono.dispose(); _email.dispose(); _contacto.dispose(); super.dispose(); }

  void _guardar() {
    if (!_formKey.currentState!.validate()) return;
    Navigator.pop(context, {
      'codigo': _codigo.text.trim(), 'nombre': _nombre.text.trim(), 'ruc': _ruc.text.trim(),
      'direccion': _direccion.text.trim(), 'telefono': _telefono.text.trim(),
      'email': _email.text.trim(), 'contacto_nombre': _contacto.text.trim(), 'activo': _activo,
    });
  }
  ...
  Widget build(BuildContext context) {
    return Form(
      ...
      child: Column(
        ...
        children: [
          AppFormSection(title: 'Datos del Proveedor', children: [
            ...
            AppTextField(controller: _contacto, label: 'Contacto', icon: Icons.person_outline),
