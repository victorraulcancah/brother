...
class _UsuariosScreenState extends State<UsuariosScreen> {
  ...
  Future<void> _openForm({Map<String, dynamic>? item, int? index}) async {
    final result = await showAppModal<Map<String, dynamic>>(
      context, title: item == null ? 'Nuevo usuario' : 'Editar usuario',
    ...
    try {
      ...
      await _load();
      if (mounted) showAppSnackbar(context, item == null ? 'Usuario creado' : 'Usuario actualizado', type: AppSnackbarType.success);
    } catch (e) { if (mounted) showAppSnackbar(context, 'Error: $e', type: AppSnackbarType.error); }
  }

  Future<void> _delete(int index) async {
    final item = _items[index];
    final confirmado = await showAppConfirmDialog(context, title: 'Eliminar usuario', message: '¿Eliminar "${item['name']}"?');
    if (!confirmado) return;
    try {
      await _crud.delete(item['id']);
      await _load();
      if (mounted) showAppSnackbar(context, 'Usuario eliminado', type: AppSnackbarType.error);
    } catch (e) { if (mounted) showAppSnackbar(context, 'Error: $e', type: AppSnackbarType.error); }
  }
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          : _items.isEmpty ? const Center(child: Text('No hay usuarios'))
          : ListView.builder(
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          AppFormSection(title: 'Datos del Usuario', children: [
            AppTextField(controller: _name, label: 'Nombre', icon: Icons.person, validator: (v) => (v == null || v.trim().isEmpty) ? 'Ingrese el nombre' : null),
            AppTextField(controller: _email, label: 'Email', icon: Icons.email_outlined, validator: (v) {
